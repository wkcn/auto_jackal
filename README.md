...
├── train.py               # 单线程训练脚本（带可视化）
├── train_parallel.py      # 多线程并行训练脚本（8个worker）
...
|---------|------|---------|----------|---------|----------|
| 单线程 | 1x | 低 | ~256MB | 无 | 资源受限、调试 |
| 单线程+跳帧(4) | ~4x | 低 | ~256MB | 无 | 资源受限、快速训练 |
| 8线程并行 | ~8x | 高 | ~2-4GB | 无 | 无人值守训练 |
| 8线程+跳帧(4) | ~32x | 高 | ~2-4GB | 无 | 最快训练速度 |
| 8线程+8画面 | ~7x | 高 | ~2-4GB | 8个实时画面 | 观看训练过程 |

**注意**：跳帧处理可以显著提升训练速度，但可能略微影响控制精度。对于大多数NES游戏，`frame_skip=4`是速度和精度的最佳平衡点。
| 8线程+8画面 | ~7x | 高 | ~2-4GB | 8个实时画面 | 观看训练过程 |
